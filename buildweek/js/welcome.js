...
let container = document.querySelector('.container')
    ...
    for(let e of questions){
        ...
        let risposte = []
        risposte.push(e.correct_answer) 
        for (let i = 0; i < e.incorrect_answers.length; i++) {
            risposte.push(e.incorrect_answers[i])
        }
        ...
        if(e.type == 'multiple'){
            for (let i = 0; i < 4; i++) {
                ...
                optionText.setAttribute('type','radio')
                optionText.style.visibility = 'hidden'

                let label = document.createElement('label')
                console.log(label.textContent = risposte[i])
        ...
        domanda.textContent = e.question
        break;
        }
    }
